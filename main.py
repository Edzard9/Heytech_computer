...
os.makedirs("qr_codes", exist_ok=True) #folder qr code
os.makedirs("bukti_pembayaran", exist_ok=True) #folder bukti pembayaran
...
class PaymentDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.total_label = QLabel(f"Rp {self.total_amount:,}")
        ...
        cash_input_layout = QHBoxLayout()
        cash_input_layout.addWidget(QLabel("Jumlah yang Dibayar:"))
        self.input_dibayar = QLineEdit()
        self.input_dibayar.setPlaceholderText("Masukkan jumlah uang yang diterima")
        self.input_dibayar.textChanged.connect(self.hitung_kembalian)
        cash_input_layout.addWidget(self.input_dibayar)
        cash_layout.addLayout(cash_input_layout)
        self.kembalian_label = QLabel("Kembalian: Rp 0")
        self.kembalian_label.setStyleSheet("font-weight: bold; color: #28a745; font-size: 14px;")
        cash_layout.addWidget(self.kembalian_label)
        self.kurang_label = QLabel("")
        self.kurang_label.setStyleSheet("color: #dc3545; font-size: 12px; font-weight: bold;")
        cash_layout.addWidget(self.kurang_label)
        ...
        form_layout.addRow(self.cash_frame)
        ...
        btn_proses.clicked.connect(self.validasi_dan_proses)
    ...
    def hitung_kembalian(self):
        try:
            dibayar_text = self.input_dibayar.text().strip()
            if not dibayar_text:
                self.kembalian_label.setText("Kembalian: Rp 0")
                self.kurang_label.setText("")
                return
            
            dibayar = int(dibayar_text.replace(',', '').replace('.', ''))
            total = self.total_amount
        
            if dibayar < total:
                kurang = total - dibayar
                self.kembalian_label.setText(f"Kembalian: Rp 0")
                self.kembalian_label.setStyleSheet("font-weight: bold; color: #dc3545; font-size: 14px;")
                self.kurang_label.setText(f"⚠️ Kurang: Rp {kurang:,}")
            else:
                kembalian = dibayar - total
                self.kembalian_label.setText(f"Kembalian: Rp {kembalian:,}")
                self.kembalian_label.setStyleSheet("font-weight: bold; color: #28a745; font-size: 14px;")
                self.kurang_label.setText("")
        except ValueError:
            self.kembalian_label.setText("Kembalian: Rp 0")
    ...
    def on_metode_changed(self, metode):
        if metode == "Cash":
            ...
            self.info_frame.setVisible(False)
            self.btn_upload_bukti.setVisible(False)
            self.label_bukti.setVisible(False)
            self.input_dibayar.setFocus()
            ...
            self.btn_upload_bukti.setVisible(True)
            self.label_bukti.setVisible(True)
        elif metode == "QRIS":
            self.cash_frame.setVisible(False)
            self.info_label.setText("Scan QRIS yang tersedia di kasir.\nBukti pembayaran kirim melalui WhatsApp 081234567890.")
            self.info_frame.setVisible(True)
            self.btn_upload_bukti.setVisible(True)
            self.label_bukti.setVisible(True)
        else:
    ...
    def validasi_dan_proses(self):
        if not self.input_admin.text().strip():
        ...
        if metode == "Cash":
            dibayar_text = self.input_dibayar.text().strip()
            if not dibayar_text:
                QMessageBox.warning(self, "Peringatan", "Jumlah yang dibayar harus diisi untuk metode Cash!")
                self.input_dibayar.setFocus()
                return
            
            try:
                dibayar = int(dibayar_text.replace(',', '').replace('.', ''))
                if dibayar < self.total_amount:
                    QMessageBox.warning(self, "Peringatan", 
                                      f"Jumlah yang dibayar kurang!\n\nTotal: Rp {self.total_amount:,}\nDibayar: Rp {dibayar:,}\nKurang: Rp {self.total_amount - dibayar:,}")
                    self.input_dibayar.setFocus()
                    return
            except ValueError:
                QMessageBox.warning(self, "Peringatan", "Format jumlah yang dibayar tidak valid!")
                self.input_dibayar.setFocus()
                return
                
        elif metode in ['Transfer Bank', 'QRIS']:
            if not self.bukti_path:
    ...
    def get_payment_data(self):
        dibayar = 0
        kembalian = 0
        if self.combo_metode.currentText() == "Cash":
            try:
                dibayar_text = self.input_dibayar.text().strip()
                if dibayar_text:
                    dibayar = int(dibayar_text.replace(',', '').replace('.', ''))
                    kembalian = max(0, dibayar - self.total_amount)
                else:
                    dibayar = self.total_amount
            except ValueError:
                dibayar = self.total_amount
...
class PrintReceiptDialog(QDialog):
    def __init__(self, transaksi_data, payment_data, parent=None):
        super().__init__(parent)
    ...
    def generate_receipt_preview(self):
        ...
        admin = self.transaksi_data.get('admin', '-')
        if admin is None:
            admin = '-'
        receipt.append(f"Admin    : {str(admin)[:12]}")
        pembeli = self.transaksi_data.get('pembeli', '-')
        if pembeli is None:
            pembeli = '-'
        receipt.append(f"Customer : {str(pembeli)[:12]}")
        metode_bayar = self.transaksi_data.get('metode_bayar', 'Cash')
        if metode_bayar is None:
            metode_bayar = 'Cash'
        receipt.append(f"Bayar    : {metode_bayar}")
        receipt.append("-" * 32)
        ...
        receipt.append("-" * 32)
        total = self.transaksi_data.get('total', 0)
        total_str = f"Rp{total:,}"
        receipt.append(f"TOTAL      : {total_str:>16}")
        if self.transaksi_data.get('metode_bayar') == 'Cash' and self.payment_data:
            dibayar = self.payment_data.get('dibayar', total)
            kembalian = self.payment_data.get('kembalian', 0)
            dibayar_str = f"Rp{dibayar:,}"
            kembalian_str = f"Rp{kembalian:,}"
            receipt.append(f"DIBAYAR    : {dibayar_str:>16}")
            receipt.append(f"KEMBALI    : {kembalian_str:>16}")
        else:
            receipt.append(f"DIBAYAR    : {total_str:>16}")
    ...
    def save_as_pdf(self):
        ...
        if file_path:
            try:
                struk_width = 80 * 2.83465  # 80mm dalam points
                struk_height = 400 * 2.83465  # 400mm dalam points
                c = canvas.Canvas(file_path, pagesize=(struk_width, struk_height))
                font_size = 8
                c.setFont("Courier", font_size)
                width, height = struk_width, struk_height
                y = height - 20 
                x = 15 
                line_length = 32 
                c.drawString(x, y, "=" * line_length)
                y -= 12
                c.drawString(x, y, "      HAYTECH STORE".center(line_length))
                y -= 12
                c.drawString(x, y, "  Toko Komputer Terpercaya".center(line_length))
                y -= 12
                c.drawString(x, y, "=" * line_length)
                y -= 15
                c.drawString(x, y, f"ID Trans : {self.transaksi_data.get('id', '-')}")
                y -= 10
                c.drawString(x, y, f"Tanggal  : {self.transaksi_data.get('tanggal', '-')}")
                y -= 10
                admin = self.transaksi_data.get('admin', '-')
                if admin is None:
                    admin = '-'
                c.drawString(x, y, f"Admin    : {str(admin)[:12]}")
                y -= 10
                pembeli = self.transaksi_data.get('pembeli', '-')
                if pembeli is None:
                    pembeli = '-'
                c.drawString(x, y, f"Customer : {str(pembeli)[:12]}")
                y -= 10
                metode_bayar = self.transaksi_data.get('metode_bayar', 'Cash')
                if metode_bayar is None:
                    metode_bayar = 'Cash'
                c.drawString(x, y, f"Bayar    : {metode_bayar}")
                y -= 15
                c.drawString(x, y, "-" * line_length)
                y -= 12
                ...
                c.drawString(x, y, "-" * line_length)
                y -= 12
                for item in self.transaksi_data.get('items', []):
                    nama = item.get('nama', '-')
                    if len(nama) > 15:
                        nama = nama[:12] + "..."
                    qty = item.get('qty', 0)
                ...
                total = self.transaksi_data.get('total', 0)
                total_str = f"Rp{total:,}"
                c.drawString(x, y, f"TOTAL      : {total_str:>16}")
                y -= 10
                if self.transaksi_data.get('metode_bayar') == 'Cash' and self.payment_data:
                    dibayar = self.payment_data.get('dibayar', total)
                    kembalian = self.payment_data.get('kembalian', 0)
                    dibayar_str = f"Rp{dibayar:,}"
                    kembalian_str = f"Rp{kembalian:,}"
                    c.drawString(x, y, f"DIBAYAR    : {dibayar_str:>16}")
                    y -= 10
                    c.drawString(x, y, f"KEMBALI    : {kembalian_str:>16}")
                else:
                    c.drawString(x, y, f"DIBAYAR    : {total_str:>16}")
                    y -= 10
                    c.drawString(x, y, f"KEMBALI    : {'Rp0':>16}")
                    
                y -= 15
                c.drawString(x, y, "=" * line_length)
                y -= 12
                c.drawString(x, y, "   TERIMA KASIH ATAS".center(line_length))
                y -= 10
                c.drawString(x, y, "   KUNJUNGAN ANDA".center(line_length))
                y -= 10
                c.drawString(x, y, "=" * line_length)
                y -= 15
                c.drawString(x, y, "*Barang yg sudah dibeli")
                y -= 10
                c.drawString(x, y, " tdk dpt ditukar/dikembalikan")
                y -= 10
                c.drawString(x, y, "*Struk sbg bukti pembayaran")
                y -= 10
                c.drawString(x, y, " yang sah")
                c.save()
...
class ModernDashboard(QMainWindow):
    ...
    def proses_pembayaran(self):
        ...
        if payment_dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                receipt_dialog = PrintReceiptDialog(transaksi_data, payment_data, self)
                receipt_dialog.exec_()
                if payment_data['metode'] == 'Cash':
                    success_msg = f"Transaksi berhasil!\n\nID Transaksi: {id_transaksi}\nTotal: Rp {total:,}\nDibayar: Rp {payment_data['dibayar']:,}\nKembalian: Rp {payment_data['kembalian']:,}"
                else:
                    success_msg = f"Transaksi berhasil!\n\nID Transaksi: {id_transaksi}\nTotal: Rp {total:,}\nMetode: {payment_data['metode']}"
    ...
    def filter_laporan(self):
        ...
        for row, transaksi in enumerate(transaksi_list):
            ...
            btn_struk = QPushButton("Struk PDF")
    ...
    def cetak_struk(self, id_transaksi):
        ...
        receipt_dialog = PrintReceiptDialog(transaksi_data, None, self)
        receipt_dialog.exec_()
    ...
    def create_page_qr_generator(self):
        ...
        btn_generate_from_barang = QPushButton("Generate QR Code dari Barang")
    ...
    def save_qr_code(self):
        ...
        try:
            if hasattr(self, 'current_qr_data'):
                parts = self.current_qr_data.split('|')
                if len(parts) > 2:
                    nama_barang = parts[2].replace(' ', '_').replace('/', '_')
                    filename = f"qr_code_{nama_barang}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                else:
